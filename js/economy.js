window.Economy = {
  buildings:{
    ...
    hatchery:{name:'Hatchery',cost:220,art:'◎'},
    breeding:{name:'Breeding Cave',cost:260,art:'∞'},
    arena:{name:'Battle Arena',cost:300,art:'⚔'}
  },
  levelNeed(level){return level*100},
  gainXP(state,amount){
    state.xp+=amount;
    while(state.xp>=this.levelNeed(state.level)){
      state.xp-=this.levelNeed(state.level);
      state.level++;
      state.gems+=3;
    }
  }
